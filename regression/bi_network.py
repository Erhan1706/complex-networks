...
class BiNetwork:
    ...
    def __init__(self, interaction_df, user_features, filter_na_users=False):
        ...
        if filter_na_users:
            ...
            user_features = user_features[~user_features['user_id'].isin(users_with_na)].reset_index(drop=True)
            interaction_df = interaction_df[~interaction_df['user_id'].isin(users_with_na)].reset_index(drop=True)
        ...
        full_network = full_network.merge(interaction_df[['user_id', 'video_id', 'timestamp', 'watch_ratio']],
